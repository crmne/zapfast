...
pub struct Glyph {
    ...
    ///
    /// ZapFast uses this to reorder bidirectional lines without guessing which
    /// glyphs belong together. Zero-width continuation glyphs use the byte
    ...
    ///
    /// Layout leaves this `false`. ZapFast sets it from the Unicode bidi algorithm
    /// so cursor hit-testing follows visual order while glyph indices stay logical.
    #[cfg_attr(feature = "serde", serde(default))]
    pub rtl: bool,
}
...
impl Galley {
    ...
    pub fn cursor_right_one_character(&self, cursor: &CCursor) -> CCursor {
        ...
        if cursor.index < self.end().index {
            CCursor {
                index: cursor.index + 1,
