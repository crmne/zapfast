...
mod tests {
    ...
    fn a_finished_card_download_is_filed_under_that_card_only() {
        ...
        // A finished download files the path it landed on; a completed file
        // in the current folder is kept verbatim.
        let dir = worker.dirs.ensure_media_dir().unwrap();
        let path = dir.join("carousel-card-1.jpg");
